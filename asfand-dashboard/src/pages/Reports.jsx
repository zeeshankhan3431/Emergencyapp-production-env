...
export default function Reports() {
  ...
  async function downloadIncidentsCsv() {
    ...
    try {
      ...
      setMessage(`Exported ${rows.length} incident(s) successfully.`);
    } catch (e) {
      ...
      <div className="p-8 space-y-6 max-w-5xl">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Export and review incident data for compliance, auditing, and operational oversight.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
          ...
          <p className="text-sm text-gray-600">
            Download a CSV of all incidents from{' '}
            <code className="text-xs bg-gray-100 px-1 rounded">GET /api/incidents</code>.
            The file includes ID, type, location, status, and timestamp.
          </p>
          <button
            type="button"
          ...
          {message ? (
            <p className={`text-sm ${message.includes('failed') || message.includes('Could') ? 'text-red-600' : 'text-green-600'}`}>
              {message}
            </p>
          ) : null}
        </div>
